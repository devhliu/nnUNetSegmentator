...
def test_install_local_model_from_directory_by_name(monkeypatch, tmp_path, isolated_registry):
    ...
    installed = TaskRegistry.install_task_models_from_local(
        task_name=task.name,
        model_sources={"model_a": source_dir},
    )

    target = tmp_path / "models" / task.name / "Dataset901_ModelA"
...
def test_install_local_model_from_zip_by_task_id(monkeypatch, tmp_path, isolated_registry):
    monkeypatch.setenv("NNUNETSEGMENTATOR_MODEL_ROOTPATH", str(tmp_path / "models"))
    task = _make_task()
    TaskRegistry.register(task)
    ...
    installed = TaskRegistry.install_task_models_from_local(
        task_name=task.name,
        model_sources={"Dataset902_ModelB": archive},
    )

    target = tmp_path / "models" / task.name / "Dataset902_ModelB"
...
def test_install_local_model_from_tar_by_name(monkeypatch, tmp_path, isolated_registry):
    monkeypatch.setenv("NNUNETSEGMENTATOR_MODEL_ROOTPATH", str(tmp_path / "models"))
    task = _make_task()
    TaskRegistry.register(task)
...
def test_install_local_model_supports_force_overwrite(monkeypatch, tmp_path, isolated_registry):
    ...
    TaskRegistry.install_task_models_from_local(
        task_name=task.name,
        model_sources={"model_a": source_v1},
    )
    TaskRegistry.install_task_models_from_local(
        task_name=task.name,
        model_sources={"model_a": source_v2},
        force=True,
    )

    target_file = tmp_path / "models" / task.name / "Dataset901_ModelA" / "version.txt"
